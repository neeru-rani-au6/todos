...
module.exports = {
    ...
    deleteTodo: async (req, res, next) => {
        try {
            ...
            const index = bucket.todos.findIndex(todo=>todo._id === todoId)
            if(index){
                bucket.todos.splice(index,1)
                await bucket.save()
            }
            return res.status(200).json({success: true, message:"Todo deleted successfully", response: bucket.todos})
        }
        catch (err) {
    ...
    updateTodo: async (req, res, next) => {
        try {
            ...
            const index = bucket.todos.findIndex(todo=>todo._id === _id)
            if(index){
                bucket.todos.splice(index,1)
                await bucket.save()
                bucket.todos.push({_id, title, isCompleted})
                await bucket.save()
            }
            return res.status(201).json({success: true, message:"todo  update successfully", response: bucket.todos})
        }
        catch (err) {
            console.log("Error in addTodo", err.message)
            return res.status(400).json({ message: `Error in addTodo ${err.message}` })
